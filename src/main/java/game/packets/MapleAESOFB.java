...
import game.packets.input.MapleEncryptionException;
...
public class MapleAESOFB {
    ...
    private static final int FIRST_CHUNK_SIZE = 0x5B0;
    private static final int NEXT_CHUNK_SIZE = 0x5B4;
    ...
    private final byte[] streamBlock = new byte[AES_BLOCK_SIZE];
    private final byte[] streamBlockScratch = new byte[AES_BLOCK_SIZE];
     ...
     */
    public MapleAESOFB(byte iv[], short mapleVersion, boolean isLoginServer) {
        final byte[] key;
        if (mapleVersion < 156) {
            key = new byte[] {0x13, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, (byte) 0xB4, 0x00, 0x00, 0x00, 0x1B, 0x00, 0x00, 0x00, 0x0F, 0x00, 0x00, 0x00, 0x33, 0x00, 0x00, 0x00, 0x52, 0x00, 0x00, 0x00};
        } else {
            key = new byte[]{(byte) 0xCA, 0x00, 0x00, 0x00, (byte) 0x89, 0x00, 0x00, 0x00, (byte) 0xFD, 0x00, 0x00, 0x00, (byte) 0xF6, 0x00, 0x00, 0x00, (byte) 0x99, 0x00, 0x00, 0x00, (byte) 0xF0, 0x00, 0x00, 0x00, (byte) 0xA6, 0x00, 0x00, 0x00, 0x0B, 0x00, 0x00, 0x00};
        }
/*
----------- AOB ----------------
...
----------- MapleShark format ----------------
13522A5B080210600602430FB44B35051B0A5F090F500C1B3355010952DEC71E
...
*/
        ...
        try {
            ...
            throw new IllegalStateException("Unable to initialize Maple AES cipher.", e);
        }
        // Set IV
        this.iv = iv; // Sets the IV of this instance.
    ...
    public byte[] crypt(byte[] data) {
        ...
        int llength = Math.min(FIRST_CHUNK_SIZE, remaining);
        int start = 4;

        try {
            while (remaining > 0) {
                byte[] inputBlock = streamBlock;
                byte[] outputBlock = streamBlockScratch;
                repeatIv(this.iv, inputBlock);

                int end = start + llength;
                int fullBlockEnd = end - AES_BLOCK_SIZE;
                while (start <= fullBlockEnd) {
                    cipher.update(inputBlock, 0, AES_BLOCK_SIZE, outputBlock, 0);
                    data[start] ^= outputBlock[0];
                    data[start + 1] ^= outputBlock[1];
                    data[start + 2] ^= outputBlock[2];
                    data[start + 3] ^= outputBlock[3];
                    data[start + 4] ^= outputBlock[4];
                    data[start + 5] ^= outputBlock[5];
                    data[start + 6] ^= outputBlock[6];
                    data[start + 7] ^= outputBlock[7];
                    data[start + 8] ^= outputBlock[8];
                    data[start + 9] ^= outputBlock[9];
                    data[start + 10] ^= outputBlock[10];
                    data[start + 11] ^= outputBlock[11];
                    data[start + 12] ^= outputBlock[12];
                    data[start + 13] ^= outputBlock[13];
                    data[start + 14] ^= outputBlock[14];
                    data[start + 15] ^= outputBlock[15];
                    start += AES_BLOCK_SIZE;

                    byte[] swapBlock = inputBlock;
                    inputBlock = outputBlock;
                    outputBlock = swapBlock;
                }
                if (start < end) {
                    cipher.update(inputBlock, 0, AES_BLOCK_SIZE, outputBlock, 0);
                    int streamIndex = 0;
                    while (start < end) {
                        data[start++] ^= outputBlock[streamIndex++];
                    }
                }

                remaining -= llength;
                llength = Math.min(NEXT_CHUNK_SIZE, remaining);
            }

            this.iv = getNewIv(this.iv);
        } catch (ShortBufferException e) {
            throw new MapleEncryptionException();
        }
        return data;
    }

    /**
     * Builds and encrypts a complete Maple packet with one final packet allocation.
     * The supplied payload is not modified.
     *
     * @param payload unencrypted packet payload
     * @return four-byte header followed by the encrypted payload
     */
    public byte[] encryptPacket(byte[] payload) {
        byte[] packet = getPacketHeaderEx(payload.length);
    ...
    private static void repeatIv(byte[] iv, byte[] output) {
        output[0] = iv[0];
        output[1] = iv[1];
        output[2] = iv[2];
        output[3] = iv[3];
    ...
    /**
     * Generates the packet header byte array of size (4 +  <code>length</code>)
     * 
     * @param length How long the packet that this header is for is.
     * @return The header byte array
     */
    public byte[] getPacketHeaderEx( int length) {
        int iiv = (iv[3]) & 0xFF;
        iiv |= (iv[2] << 8) & 0xFF00;
    ...
    public boolean checkPacket(int packetHeader) {
        ...
        return (((((packetHeader >>> 24) ^ iv[2]) & 0xFF) == ((mapleVersion >> 8) & 0xFF))
                && ((((packetHeader >>> 16) ^ iv[3]) & 0xFF) == (mapleVersion & 0xFF)));
    }
    ...
    private static byte[] getNewIv(byte[] oldIv) {
        byte[] output = {(byte) 0xf2, 0x53, 0x50, (byte) 0xc6};
        funnyShit(oldIv[0], output);
        funnyShit(oldIv[1], output);
        funnyShit(oldIv[2], output);
        funnyShit(oldIv[3], output);
        return output;
    }
     ...
     */
    public static final byte[] funnyShit(byte inputByte, byte[] in) {
        byte elina = in[1];
        byte anna = inputByte;
